...
class DDPG(object):
    ...
    def update_actor_imitation(self, batch, expert):
        state_batch = torch.cat(batch.state).to(self.device)
        p_state_batch = torch.cat(batch.p_state).to(self.device)
        self.actor_optim.zero_grad()
        action_batch_n, logit = self.select_action(
            state_batch.view(-1, self.obs_dim), action_noise=self.train_noise, grad=True)
        with torch.no_grad():
            action_batch_n_ex = expert.select_action(
                p_state_batch.view(-1, self.obs_dim), action_noise=False)

        action_batch_n = action_batch_n.view(-1, self.n_action * self.n_agent)
        action_batch_n_ex = action_batch_n_ex.view(-1, self.n_action * self.n_agent)

        self.actor_optim.zero_grad()
        loss = torch.mean((action_batch_n_ex - action_batch_n)**2)
        loss.backward()
        self.actor_optim.step()
        return loss.item()

    def update_actor_advisor(self, batch, expert, aux, alpha, use_static_weight=False):
        state_batch = torch.cat(batch.state).to(self.device)
        p_state_batch = torch.cat(batch.p_state).to(self.device)
        self.actor_optim.zero_grad()

        # IL & compute weight part
        action_batch_n, logit = self.select_action(
            state_batch.view(-1, self.obs_dim), action_noise=self.train_noise, grad=True)
        with torch.no_grad():
            action_batch_n_ex = expert.select_action(
                p_state_batch.view(-1, self.obs_dim), action_noise=False)
            action_batch_n_aux = aux.select_action(
                state_batch.view(-1, self.obs_dim), action_noise=False)

        action_batch_n = action_batch_n.view(-1, self.n_agent, self.n_action)
        action_batch_n_ex = action_batch_n_ex.view(-1, self.n_agent, self.n_action)
        action_batch_n_aux = action_batch_n_aux.view(-1, self.n_agent, self.n_action)

        d0 = torch.sum((action_batch_n_aux - action_batch_n_ex)**2, dim=-1)
        if use_static_weight:
            use_expert_weights = torch.ones_like(d0, device=d0.device) * 0.5
        else:
            use_expert_weights = torch.exp(-alpha * d0).detach()
        use_rl_weights = 1 - use_expert_weights

        weighted_main_expert_loss = (
                use_expert_weights * torch.sum((action_batch_n_ex - action_batch_n)**2, dim=-1)
        ).mean()
        ...
        action_batch_n = action_batch_n.view(-1, self.n_action * self.n_agent)
        policy_loss = -self.critic(state_batch, action_batch_n)
        avg_use_rl_weights = torch.mean(use_rl_weights, dim=-1)
        weighted_policy_loss = (avg_use_rl_weights * (policy_loss + 1e-3 * (logit ** 2).mean())).mean()
        total_loss = weighted_main_expert_loss + weighted_policy_loss
        ...
        return total_loss.item(), use_expert_weights.mean().item()
